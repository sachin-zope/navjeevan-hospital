...
public class BillDao {
	...
	public void update(DTOBill dtoBill) {
		Connection conn = null;
		try {
			conn = DbUtil.getConnection();
			conn.setReadOnly(false);
	...
	public DTOBill generateBill(String roomType, DTOIndoorRegister indoorRegister) {
		...
		case "general":
			...
			break;
		case "special":
			roomCharges = getCharges("special_room_charges");
			break;
		...
		int otCharges = 0;
		int otherCharges = 0;
		int operationCharges = 0;
		
		String treatment = indoorRegister.getTreatment();
		if (treatment.equalsIgnoreCase("Delivery")) {
			...
			operationCharges = fees - (indoorCharges + consultantCharges + nursingCharges + otCharges); 
		}
	...
	public DTOBillPrint getBillToPrint(int indoorId) {
		Connection conn = null;
		try {
			conn = DbUtil.getConnection();
			conn.setReadOnly(true);
			...
			LazyList<AJBill> ajBills = AJBill.find("indoor_register_id=?", indoorId);
			if (!ajBills.isEmpty()) {
				AJBill ajBill = ajBills.get(0);
				AJIndoorRegister ajIndoorRegister = AJIndoorRegister.findById(indoorId);

				int deliveryId = ajIndoorRegister.getInteger(AJIndoorRegister.DELIVERY_REGISTER_ID) != null
						? ajIndoorRegister.getInteger(AJIndoorRegister.DELIVERY_REGISTER_ID) : 0;
				int mtpId = ajIndoorRegister.getInteger(AJIndoorRegister.MTP_REGISTER_ID) != null
						? ajIndoorRegister.getInteger(AJIndoorRegister.MTP_REGISTER_ID) : 0;
				int otId = ajIndoorRegister.getInteger(AJIndoorRegister.OT_REGISTER_ID) != null
						? ajIndoorRegister.getInteger(AJIndoorRegister.OT_REGISTER_ID) : 0;
				Date operationDate = null;
				if (deliveryId > 0) {
					LazyList<AJDeliveryRegister> ajDeliveryRegisters = AJDeliveryRegister
							.findBySQL("select delivery_date from delivery_register where id = ?", deliveryId);
					if (!ajDeliveryRegisters.isEmpty()) {
						operationDate = ajDeliveryRegisters.get(0).getDate(AJDeliveryRegister.DELIVERY_DATE);
					}
				} else if (mtpId > 0) {
					LazyList<AJMtpRegister> ajMtpRegisters = AJMtpRegister
							.findBySQL("select operation_date from mtp_register where id = ?", mtpId);
					if (!ajMtpRegisters.isEmpty()) {
						operationDate = ajMtpRegisters.get(0).getDate(AJMtpRegister.OPERATION_DATE);
					}
				} else if (otId > 0) {
					LazyList<AJOTRegister> ajotRegisters = AJOTRegister
							.findBySQL("select operation_date from ot_register where id = ?", otId);
	...
	public DTOBillReceipt generateReceipt(int indoorId) {
		Connection conn = null;
		try {
			conn = DbUtil.getConnection();
			conn.setReadOnly(true);
	...
	public DTOBill getBillByIndoorId(int indoorId) {
		Connection conn = null;
		try {
			conn = DbUtil.getConnection();
			conn.setReadOnly(true);
			...
			if (!ajBills.isEmpty()) {
				AJBill ajBill = ajBills.get(0);
				return new BillTransformer().tranform(ajBill);
			}
		} catch (Throwable t) {
			Base.rollbackTransaction();
			t.printStackTrace();
